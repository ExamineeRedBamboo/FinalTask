...
#include "MotorControl.hpp" //内含电机类
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan) {
  if (hcan == &hcan1) {
    if (HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO0, &rx_header1, can1_rx_data) ==
        HAL_OK) // 获得接收到的数据头和数据
    {
      if (rx_header1.StdId == 0x12) { // 帧头校验
        yaw.fbData[0] = can1_rx_data[1] << 8 | can1_rx_data[2]; //位置
        yaw.fbData[1] = can1_rx_data[3] << 4 | can1_rx_data[4] >> 4; //速度
      }
    }
  }
  ...
  if (hcan == &hcan2) {
        ...
        HAL_OK) // 获得接收到的数据头和数据
    {
      if (rx_header2.StdId == 0x201) { // 帧头校验
        motor.realSpeeds[0] = (int16_t)(can2_rx_data[2] << 8 | can2_rx_data[3]); // 电机1速度
      } else if (rx_header2.StdId == 0x202) {
        motor.realSpeeds[1] = (int16_t)(can2_rx_data[2] << 8 | can2_rx_data[3]); // 电机2速度
      } else if (rx_header2.StdId == 0x203) {
        motor.realSpeeds[2] = (int16_t)(can2_rx_data[2] << 8 | can2_rx_data[3]); // 电机3速度
      } else if (rx_header2.StdId == 0x204) {
        motor.realSpeeds[3] = (int16_t)(can2_rx_data[2] << 8 | can2_rx_data[3]); // 电机4速度
      }
    }
  }
